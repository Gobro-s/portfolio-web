...
const projectList: Project[] = [
  ...
  },
  {
    ...
    why: [
      "방탈출 카페는 테마 정보, 후기, 예약이 전부 다른 곳에 흩어져 있었습니다.",
      "하지만 진짜 불편은 정보가 없는 게 아니라, 마음에 드는 테마를 찾은 다음 예약하려면 앱을 나가야 한다는 것이었습니다.",
      "찾기와 예약을 한 화면에서 끝내는 앱을 만들었습니다.",
      "디자이너·PM 없는 2인 프론트라, Atomic Design과 CDD로 병렬 제작해 개발 시간을 약 50% 줄였습니다.",
    ],
    architecture: [
  ...
  },
  {
    ...
    why: [
      "직원 일정과 교육 신청이 엑셀과 수기로 흩어져 있었습니다.",
      "하지만 손이 제일 많이 가는 쪽은 외부 교육이었습니다 — 연간 일정이 외부 사이트 표에만 있어 사람이 옮겨 적어야 했습니다.",
      "전 직원이 같은 캘린더를 보고, 외부 일정은 Puppeteer 크롤러가 채우고, 정원은 Firestore 트랜잭션이 지킵니다.",
      "기획부터 배포까지 약 2주. 지금도 근무 현장에서 쓰고 있습니다.",
    ],
    architecture: [
  ...
  },
  {
    ...
    highlights: [
      ...
      {
        title: "슬롯 기반 대기열 — 호출받고 오는 중인 팀도 자리를 차지합니다",
        body: [
          "상시 게시되는 태블릿 화면이 꺼지지 않도록 Wake Lock API를 적용했습니다.",
          "정원(테이블 10석)은 '입장한 인원 + 호출을 받고 오는 중인 인원'의 합으로 셉니다.",
          "오는 중인 팀을 빼고 세면 같은 자리에 두 팀을 부르게 되고, 화면의 대기 순번도 실제보다 짧게 안내됩니다.",
          "자리가 난 만큼만 등록 순서(FIFO)대로 호출하고, 문자에 11분 입장 기한을 적습니다. 그 11분이 관람객이 걸어 돌아오는 시간입니다.",
          "퇴장 처리와 대기자 삭제 두 경로가 같은 재호출 함수를 타게 해, 빈자리가 어느 쪽에서 생기든 다음 팀이 자동으로 호출되게 했습니다.",
        ],
        images: [
...
export const aiPractice = {
  ...
  // 서명("Claude의 평가")을 뗐다. 여섯 줄이 전부 1인칭인데 AI가 평가한 것처럼 서명하면
  // 화자를 알 수 없고, 자기 평가를 제3자 검증처럼 포장한 것으로 읽힌다 —
  // PRODUCT.md가 절대 제약으로 잡은 "과장·미검증 클레임 금지"에 정면으로 걸린다.
  //
  // 순서도 뒤집었다. 2026년에 "AI를 잘 쓴다"는 변별력이 거의 없다. 변별력은
  // 얼마나 쓰냐가 아니라 어디서 멈추냐에 있으므로, 데이터 경계와 검증이 맨 위로 온다.
  // 네 번째 줄이었던 "기획 2주 만에 실운영 배포"는 원칙이 아니라 실적이라 증거로 내렸다.
  // 나머지 셋은 전부 지키는 규칙인데 그것만 결과값이어서 목록의 문법이 깨졌다.
  principles: [
    "실제 개인정보나 기관 문서는 AI에 그대로 넣지 않습니다.",
    "결과의 구조와 보안을 직접 검증한 뒤에만 실운영에 올립니다.",
    "이상하면 그 자리에서 확인을 요구합니다 — 그럴듯한 결과보다 실측을 믿습니다.",
  ],
  boundary: "모델을 만드는 ML 엔지니어는 아닙니다. 모델로 현장의 문제를 해결하는 쪽입니다.",
  // 길게 쓰면 안 읽힌다. 각 항목은 한 줄로 박고, 이유는 설명을 붙이는 대신
  // 대비로 낸다 — "AI로 2주"와 "검증은 직접"이 한 화면에 나란히 있는 식.
  evidence: [
    {
      title: "실운영 시스템 2개",
      body: [
    ...
    {
      // "보고용으로 올라간다"는 전언이라 뺐다. 본인이 직접 한 행위(출력·전달)까지만 쓴다 —
      // 면접에서 확인받을 수 있는 선이 여기까지다.
      title: "업무 문서에도 같은 순서",
      body: [
        "근태 시트도 초안은 AI, 검산은 직접입니다.",
        "시청 점검 때 이 시트로 만든 근태표를 출력해 점검 담당자에게 전달했습니다.",
        "받는 사람이 검산할 수 있도록 법정공휴일 자동 집계에 근거 출처를 답니다.",
      ],
      image: {
    ...
    {
      // 첫 줄이 원칙 1번과 거의 축자 반복이라 뺐다. 증거란이 원칙을 되풀이하면
      // 근거가 아니라 반복이다 — 실제로 어떻게 하는지만 남긴다.
      title: "다루는 데이터에는 선을 긋습니다",
      body: [
        "관람객 전화번호는 내부망 밖으로 내보내지 않으려고 설계했습니다.",
        "그 데이터를 프롬프트에 붙여넣으면 그 설계가 무의미해집니다. 구조만 남기고 익명화해서 씁니다.",
      ],
    },
    {
      // 이 항목이 "실측을 믿는다"는 원칙의 실제 근거다. 주석에만 두면 아무도 못 본다.
      title: "AI의 진단을 실측으로 되돌린 기록",
      body: [
        "이 사이트 이미지가 하나씩 뜨자 AI는 네트워크 병렬처리를 원인으로 짚었습니다.",
        "직접 재보니 요청은 이미 병렬이었습니다. 진짜 원인은 스크롤이 지연 로딩 판정을 흔든 것이었습니다.",
      ],
    },
    {
      title: "병목은 프롬프트가 아니라 요구사항",
      body: [
        "Open The Door에서 기획 → 와이어프레임 → 기능명세서 → 프로토타입 단계를 리드했습니다.",
        "현장의 규칙을 명세로 바꾸는 능력이 위임의 품질을 정합니다.",
      ],
    },
    {
      title: "이 사이트도 그렇게 만들었습니다",
      body: ["Claude Code와 함께 설계·구현·배포했습니다. 지금 보고 있는 화면이 증거입니다."],
    },
  ],
};
 ...
 * 현장 사업 — /field 전용.
 *
 * 개발 포트폴리오와 축이 다르다. 여기 실리는 것은 "무엇을 만들었나"가 아니라
 * "기술을 모르는 상대에게 기술을 설명하고 현장에 걸어봤나"다. 기술영업·프리세일즈·
 * 기술지원 지원 시에만 이 URL을 낸다. 랜딩에서는 링크 한 줄로만 존재한다.
 *
 * 🔴 이 페이지의 모든 항목은 서울로봇인공지능과학관 **업무분장표에 있는 분장사무**다.
 *    분장에 없는 일을 여기 올리면 경력증명서와 어긋나 면접에서 뚫린다.
 *    - ① 기업협업 체험교육 '라임 PLAY' 운영지원
 *    - ③ 신규 협업기업 발굴 업무 **지원**
 *    - ⑦ 교육·문화행사 운영지원
 * 🔴 `scope`는 장식이 아니다. 분장사무 ③의 끝이 "지원"이므로 계약 주체가 아니고,
 *    "발굴했습니다"는 거짓이 된다. 매출·수주 숫자는 없으므로 영업 실적으로 쓰지 않는다.
 * 🔴 `status: "진행 중"`인 건은 결과를 쓰지 않는다. 끝나면 그때 결과를 채운다.
 */
export const fieldWork = {
  title: "현장에서, 기술을 설명하는 일",
  lead: [
    "만드는 일만 한 것은 아닙니다.",
    "기업·기관 담당자와 관람객을 직접 마주하고, 로봇과 교육 프로그램이 무엇을 할 수 있는지 설명하고 현장에 걸어 왔습니다.",
  ],
  // 스스로 선을 긋는 문장. 이게 없으면 아래 고객사 이름들이 영업 실적처럼 읽힌다.
  boundary:
    "아래는 전부 서울로봇인공지능과학관의 배정 업무이고, 저는 운영과 발굴을 지원하는 자리에 있습니다. 제안서 작성과 계약은 주무관·팀장이 맡습니다. 매출이나 수주 실적이 아닙니다.",
  items: [
    {
      name: "현대백화점 판교점 팝업스토어",
      partner: "현대백화점 마케팅 담당자",
      period: "2026 (진행 중)",
      inProgress: true,
      summary: "휴닛 로봇팔 체험과 그림 그리는 교육 프로그램을 백화점 매장에서 운영합니다.",
      body: [
        "로봇팔이 무엇을 하는지, 교육 프로그램이 어떻게 굴러가는지를 백화점 마케팅 담당자에게 설명하고 함께 운영 방식을 맞추고 있습니다.",
        "현장에서는 프로그램을 직접 제공합니다. 로봇을 처음 보는 방문객에게 설명하는 일이 그대로 업무입니다.",
      ],
      scope: "분장사무 ③ 신규 협업기업 발굴 업무 지원 · ① 라임 PLAY 운영지원",
    },
    {
      name: "과학관 개관 2주년 행사",
      partner: "KT · 도봉경찰서",
      period: "2026 (진행 중)",
      inProgress: true,
      summary: "KT, 도봉경찰서와 함께 개관 2주년 행사를 준비하고 있습니다.",
      body: [
        "기업·공공기관 각각과 협의해 행사 구성을 맞추고 있습니다.",
        "말이 통하는 방식이 서로 다릅니다. 같은 프로그램을 통신사 담당자와 경찰서 담당자에게 각각 다른 언어로 설명해야 했습니다.",
      ],
      scope: "분장사무 ⑦ 교육·문화행사 운영지원",
    },
    {
      name: "전시 체험실 리모델링",
      partner: "과학관 내부 · 협력사",
      period: "2026 (진행 중)",
      inProgress: true,
      summary: "체험 전시실 리모델링 업무를 진행하고 있습니다.",
      body: [
        "체험실을 매일 운영해 본 사람으로서 현장에서 실제로 막히는 지점을 반영하고 있습니다.",
      ],
      scope: "분장사무 ⑦ 교육·문화행사 운영지원",
    },
    {
      name: "신규 협업기업 발굴 지원",
      partner: "라임 PLAY 기업협업 체험교육",
      period: "2026",
      inProgress: false,
      summary: "협업 후보 기업을 조사하고 정리해 담당자에게 넘기는 일을 맡았습니다.",
      body: [
        "후보 기업을 소팅하고 6곳을 조사했으며, 조건이 맞지 않는 1곳을 제외 의견과 함께 정리했습니다.",
        "제안과 계약은 주무관·팀장이 맡고, 저는 그 앞단의 조사와 정리를 담당했습니다.",
      ],
      scope: "분장사무 ③ 신규 협업기업 발굴 업무 지원",
    },
  ],
